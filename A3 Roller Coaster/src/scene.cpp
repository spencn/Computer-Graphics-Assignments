...

{
        ...
        vec3 o, x, y, z;
        float dataSize = spline->data.size();
        for (int p = 0; p < dataSize; p++) {
            for (int i = 0; i < 200; i++) {
                float t = p + (i / 200.0f);
                mat4 M = spline->findLocalTransform(t);
                M = M * translate(2, 0, 0) * scale(1.0, 1.0, 1.0);
                mat4 xMV = MV * M;
                mat4 xMVP = MVP * M;
                cylinder->draw(xMV, xMVP, lightDir, vec3(200 / 255.0, 100 / 255.0, 0 / 255.0));
                M = M * translate(-4, 0, 0) * scale(1.0, 1.0, 1.0);
                xMV = MV * M;
                xMVP = MVP * M;
                cylinder->draw(xMV, xMVP, lightDir, vec3(200 / 255.0, 100 / 255.0, 0 / 255.0));
            }
        ...
        for (int p = 0; p < dataSize; p++) {
            for (int i = 0; i < DIST_BETWEEN_TIES; i++) {
                float t = p + (i / DIST_BETWEEN_TIES);
                mat4 M = spline->findLocalTransform(t);
                M = M * rotate(2.2f, vec3(1, 1, 1)) * scale(1.0, 1.0, 3.0);
                mat4 xMV = MV * M;
                mat4 xMVP = MVP * M;
                cylinder->draw(xMV, xMVP, lightDir, vec3(200 / 255.0, 100 / 255.0, 0 / 255.0));
            }
    ...
    //for (int p = 0; p < dataSize; p++) {
    //    for (int i = 0; i < 10; i++) {
    //        //cout << p + (i / 100.0f) << endl;
    //        float t = p + (i / 10.0f);
    //        spline->findLocalSystem(t, o, x, y, z);
    //        o.x += 3;
    //        mat4 M = translate(o) * rotate(3, vec3(0, 1, 1)) * scale(1.0, 1.0, 1.0);
    //        mat4 xMV = MV * M;
    //        mat4 xMVP = MVP * M;
    //        cylinder->draw(xMV, xMVP, lightDir, vec3(200 / 255.0, 100 / 255.0, 0 / 255.0));
    ...
    //        spline->findLocalSystem(t, o, x, y, z);
    //        o.x -= 3;
    //        M = translate(o) * rotate(3, vec3(0, 1, 1)) * scale(1.0, 1.0, 1.0);
    //        xMV = MV * M;
    //        xMVP = MVP * M;
    //        cylinder->draw(xMV, xMVP, lightDir, vec3(200 / 255.0, 100 / 255.0, 0 / 255.0));
    //    }
    ...
    //vec3 o, x, y, z;
    //spline->findLocalSystem(0, o, x, y, z);
    //o.x += 3;
    //mat4 M = translate(o) * rotate(3, vec3(0, 1, 1)) * scale(1.0, 1.0, 1.0);
    //mat4 xMV = MV * M;
    //mat4 xMVP = MVP * M;
