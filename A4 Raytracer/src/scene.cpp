...
vec3 Scene::raytrace( vec3 &rayStart, vec3 &rayDir, int depth, int thisObjIndex, int thisObjPartIndex, float weight )

{
    ...
    //Total specular coeffecient (product of previous) is added to the parameters
    //if this is less than the threshold, apply russian roulette
    //Specular coeffecient k
    //w* = weight the ray should be
    //w = actual weight
    //if w*>= the threshold continue and set w = w*
    //else terminate, set w=0
    //RR threshold 0.126
    //Probability of RR 0.5
    // Find the closest object intersected

    vec3     Prr, Nrr, texcoordsrr;
    float    trr;
    int      objIndexrr, objPartIndexrr;
    Material* matrr;
    float randrr;
    float newWeight = weight;
    float threshold = 0.01;
    float terminationProb = 0.5;
    bool hitrr = findFirstObjectInt(rayStart, rayDir, thisObjIndex, thisObjPartIndex, Prr, Nrr, texcoordsrr, trr, objIndexrr, objPartIndexrr, matrr, -1);
    if (hitrr) {
        randrr = randIn01();
        float k = matrr->ks.x;
        if (k == 0) {
        ...
        newWeight = weight * k;
        if (newWeight < threshold & randrr < terminationProb) {
            
            newWeight = 0;
            return blackColour;
        }
        else if (newWeight < threshold & randrr >= terminationProb) {
            newWeight = (1.0 / (1.0 - terminationProb)) * newWeight;
    ...
    vec3 Iin = raytrace( P, R, depth, objIndex, objPartIndex, newWeight );
        ...
        if (i != thisObjIndex) {
            ...
            if (tri && tri->mat->Ie.squaredLength() > 0) {
                ...
                for(int j=0; j < NUM_SOFT_SHADOW_RAYS; j++){
                    ...
                    vec3 triangleLight = x * triangle->verts[0].position + y * triangle->verts[1].position + (1-x-y) * triangle->verts[2].position;
                    vec3 L = triangleLight - P;
                    ...
                    if (N*L > 0) {
                        ...
                        if (found && intObjIndex == i) {
                            vec3 Lr = (2 * (L * N)) * N - L;
                            Iout = Iout + calcIout( N, L, E, Lr, kd, mat->ks, mat->n, (1.0/NUM_SOFT_SHADOW_RAYS) * triangle->mat->Ie);
                        }
                    }
                }
            }
...

{
  ...
  // is false; use a jittered patter if 'jitter' is true.
    ...
    int p = numPixelSamples * numPixelSamples;
    vec3 sum(0,0,0);
    vec3 dir = (llCorner + (x+0.5)*right + (y+0.5)*up).normalize();
    
    /// Use a regular pattern in the subpixel centres if 'jitter' is false; use a jittered patter if 'jitter' is true.
    for(int i=0; i < p; i++) {
        ...
        if(jitter) {
            x1 = static_cast <float>( rand() ) / RAND_MAX;
        ...
        /// find ray to center of each subdivision
        x2 = (i % numPixelSamples - static_cast <float>( (numPixelSamples - 1) )/2);
        y2 = (i / numPixelSamples - static_cast <float>( (numPixelSamples - 1) )/2);
        
        /// update jitter direction around center of each subdivision
        dir = dir + x2 / numPixelSamples * right + y2 / numPixelSamples * up;
        dir = dir + x1 / numPixelSamples * right + y1 / numPixelSamples * up;
        
        /// sum each ray within a pixel
        sum = sum + raytrace( eye->position, dir, 0, -1, -1, 1);
    }

#endif
    
    /// average of all the colours coming through the pixel
    result = (1.0 / p) * sum;
